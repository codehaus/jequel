...
public class LineTransformerTest extends TestCase {
    ...
    public void testTransformSelect() {
        ...
        assertEquals("(sub_select( )", transformer.transformKeywords("(select "));
        assertEquals("Select( a ).from( b ).where( c ).order_by( d)", transformer.transformKeywords("select a from b where c order by d"));
    }
    ...
    public void testUpcaseIdentfiersSchemaAlias() {
        assertEquals("from ARTICLE_COLOR.as(\"article_color_alias\")",
                transformer.upcaseIdentifiersForSchema("from article_color article_color_alias"));
    }
    ...
    public void testAll() {
        ...
        assertEquals("Select( a,b ).from( c,d ).where( c.a .eq( b.a ).and( c.a .in(1,2,3)).or( d.d .is_not( null ).order_by( a)", sql);
    }
